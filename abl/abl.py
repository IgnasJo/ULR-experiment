...
class ABL(nn.Module):
    ...
    def get_direction_gt_predkl(self, pred_dist_map, pred_bound, logits):
        ...
        pred_dist_map_d = torch.nn.functional.pad(pred_dist_map, (1, 1, 1, 1, 0, 0), mode='constant', value=max_dis)
        logits_d = torch.nn.functional.pad(logits, (0, 0, 1, 1, 1, 1, 0, 0), mode='constant')
        ...
        # allocate on-device (no hardcoded .cuda())
        dist_maps = torch.zeros((0, len(x)), device=device, dtype=pred_dist_map.dtype)  # 8k
        kl_maps = torch.zeros((0, len(x)), device=device, dtype=logits.dtype)          # 8k
